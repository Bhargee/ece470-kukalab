...
class image_converter:
  ...
  def callback(self,data):
    ...
    lower_cube = np.array([110,50,50])
    upper_cube = np.array([130,255,255])
    lower_dots = np.array([35,23,104])
    upper_dots = np.array([55,153,255])
    ...
    for contour in dots_contours:
      ...
      if (w*h) >= 75:
        ...
        cv2.putText(cv_image_copy,'%s,%s'%(x+(w/2),y+(h/2)),(x,y), font, .5,(0,255,0),2)
    ...
    for contour in cube_contours:
      ...
      if (w*h) >= 250:
        cv2.circle(cv_image_copy, (x+(w/2),y+(h/2)), 1, (0,255,0), 3)
        cv2.rectangle(cv_image_copy, (x,y), (x+w, y+h), (255,0,0), 2)
        cv2.putText(cv_image_copy,'%s,%s'%(x+(w/2),y+(h/2)),(x,y), font, .5,(255,0,0),2)

    cv2.imshow("Image", cv_image)
    cv2.imshow("Segmented Image", cv_image_copy)
    cv2.waitKey(0)
